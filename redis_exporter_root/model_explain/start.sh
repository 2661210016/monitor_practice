...
WORKDIR=/opt/redis_exporter_root
...
nohup $WORKDIR/bin/redis_exporter   --redis.addr=redis://${IP}:${REDIS_PORT}   --web.listen-address=:${EXPORTER_PORT} > $WORKDIR/log/redis_exporter_${REDIS_PORT}.log 2>&1 &
